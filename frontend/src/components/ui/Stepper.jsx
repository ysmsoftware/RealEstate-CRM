            ...
            {steps.map((step, idx) => {
                ...
                return (
                    <React.Fragment key={idx}>
                        <div
                            className={`flex flex-col items-center flex-shrink-0 ${isClickable ? "cursor-pointer" : "cursor-default"}`}
                            onClick={() => isClickable && onStepClick && onStepClick(idx)}
                        >
                            <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold transition ${idx < currentStep
