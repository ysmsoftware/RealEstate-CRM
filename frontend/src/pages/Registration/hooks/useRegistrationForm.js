...
export function useRegistrationForm() {
    ...
    useEffect(() => {
        ...
        if (!wingForm.manualFloorEntry) {
            const targetCount = parseInt(wingForm.noOfFloors) || 0
            // We want floors 0 to targetCount (inclusive), so total items = targetCount + 1
            const targetLength = targetCount + 1
            const currentLength = currentWingFloors.length
            ...
            if (targetLength > currentLength) {
                ...
                setCurrentWingFloors([...currentWingFloors, ...newFloors])
            } else if (targetLength < currentLength) {
                // Remove excess floors from the end
                // Only if we actually have floors to remove (safety check)
                setCurrentWingFloors(currentWingFloors.slice(0, targetLength))
            }
        }
